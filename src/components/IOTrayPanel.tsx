...
export default function IOTrayPanel() {
  const { currentLevel, dockedIO, setArmedIO, armedIOId } = useStore();
  ...
  return (
    <div className="io-tray panel">
      ...
      <div className="io-tray-list">
        {items.map(g => (
          <div key={g.id} className="io-tray-item">
            ...
            <button className="io-tray-place" onClick={() => setArmedIO(armedIOId === g.id ? null : g.id)}>
              {armedIOId === g.id ? 'Cancel' : 'Place'}
            </button>
          </div>
        ))}
        {items.length === 0 && (
